...
<div class="mb-6">
    <a href="<?= route_to('product.stock.manage', $product->id) ?>" class="text-indigo-400 hover:text-indigo-300 text-sm">
        <i class="fa-solid fa-arrow-left mr-1.5"></i> Kembali ke Daftar Varian
    </a>
</div>

<h1 class="text-2xl font-semibold text-gray-100 mb-2">Kelola Item Stok</h1>
<p class="text-md text-gray-400 mb-6">
    Produk: <span class="font-medium text-white"><?= esc($product->product_name) ?></span> /
    Varian: <span class="font-medium text-white"><?= esc($variant->name) ?></span>
...
<div class="flex flex-col lg:flex-row gap-8">
    ...
    <div class="lg:w-1/3">
        <div class="bg-gray-900/80 backdrop-blur-sm border border-gray-700/50 rounded-2xl shadow-xl p-6 sticky top-8">
            <h2 class="text-xl font-semibold text-white mb-4">Tambah Item Stok Baru (Format JSON)</h2>
...
{"email": "akun2@mail.com", "password": "pass2"}</pre>
            ...
            <?= form_open(route_to('product.variant.stock.add', $product->id, $variant->id)) ?>
                 <?= csrf_field() ?>
                <div class="mb-4">
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-5 py-2 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-bold shadow-lg transition duration-300">
                        <i class="fa-solid fa-plus mr-1.5"></i> Tambah Item Stok
                    </button>
                </div>
            <?= form_close() ?>
        </div>
    ...
    <!-- Kolom Kanan: Daftar Stok (Kode Tampilan Tabel sama seperti sebelumnya) -->
     <div class="lg:w-2/3">
         <h2 class="text-xl font-semibold text-gray-300 mb-6">Daftar Item Stok untuk Varian: <?= esc($variant->name) ?></h2>
             ...
             <div class="overflow-x-auto">
                  <table class="min-w-full divide-y divide-gray-700/50">
                     ...
                     <tbody class="divide-y divide-gray-700/50">
                         <?php if (empty($stocks)): ?>
                             <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">Belum ada item stok.</td></tr>
                         <?php else: ?>
                             <?php
                             ...
                             <?php foreach ($stocks as $index => $stock): ?>
                                 ...
                                 <tr class="hover:bg-gray-800/50 transition-colors <?= $stock->is_used ? 'opacity-60' : '' ?>">
                                     ...
                                     <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium align-top">
                                         <?= form_open(route_to('product.variant.stock.delete', $product->id, $variant->id, $stock->id), ['class' => 'inline', 'onsubmit' => 'return confirm(\'Hapus item stok ini?\')']) ?>
                                             <?= csrf_field() ?>
                                             <button type="submit" class="text-red-400 hover:text-red-300 disabled:opacity-50 disabled:cursor-not-allowed" title="Hapus Item Stok" <?= $stock->is_used ? 'disabled' : '' ?>>
